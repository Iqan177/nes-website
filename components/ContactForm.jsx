...
export default function ContactForm() {
  const { lang } = useLang();
  const isDE = lang === "de";
  const interests = isDE ? INTERESTS_DE : INTERESTS_EN;
  ...
  if (status === "success") {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-petrol/10 bg-pearl-50 p-10 lg:p-14 text-center">
        ...
        <p className="text-petrol/70 max-w-md mx-auto">{isDE ? "Wir haben Ihre Anfrage erhalten und arbeiten bereits daran. Sie erhalten in Kürze eine Bestätigung per E-Mail." : "We have received your request and are already working on it. You will shortly receive a confirmation by email."}</p>
      </motion.div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-petrol/10 bg-pearl-50 p-7 lg:p-10">
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <Field label={isDE ? "Unternehmen" : "Company"} value={form.company} onChange={(v) => update("company", v)} placeholder="Optional" />
        <Field label="E-Mail" required type="email" value={form.email} onChange={(v) => update("email", v)} placeholder="ihre@adresse.de" />
        <Field label={isDE ? "Telefon" : "Phone"} type="tel" value={form.phone} onChange={(v) => update("phone", v)} placeholder="Optional" />
      </div>
      <div className="mt-7">
        <label className="font-mono text-[10px] uppercase tracking-[0.18em] text-petrol/50 mb-3 block">{isDE ? "Thema" : "Topic"}</label>
        <div className="flex flex-wrap gap-2">
          {interests.map((opt) => (
            <button type="button" key={opt} onClick={() => update("interest", opt)}
              className={`px-4 py-2 rounded-full text-[13.5px] font-medium border transition-all ${form.interest === opt ? "bg-petrol text-pearl border-petrol" : "bg-transparent text-petrol/70 border-petrol/15 hover:border-petrol/40"}`}>{opt}</button>
          ))}
        </div>
      </div>
      <div className="mt-7">
...
function Field({ label, value, onChange, type = "text", placeholder, required }) {
  return (
    <div>
      <label className="font-mono text-[10px] uppercase tracking-[0.18em] text-petrol/50 mb-2 block">{label} {required && <span className="text-cyan-700">*</span>}</label>
      <input required={required} type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-transparent border-b border-petrol/15 focus:border-cyan-700 outline-none py-3 text-[15px] text-petrol placeholder:text-petrol/40 transition-colors" />
    </div>
  );
}

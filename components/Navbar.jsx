...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? "backdrop-blur-md bg-pearl/85 border-b border-petrol/10" : "bg-transparent"
    }`}>
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 h-16 lg:h-20 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <button onClick={toggle}
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-petrol/15 hover:border-petrol/40 transition-all text-[13px] font-mono font-medium text-petrol/70 hover:text-petrol">
        ...
        <button onClick={() => setOpen(!open)}
          className="lg:hidden w-10 h-10 flex items-center justify-center rounded-full hover:bg-petrol/5" aria-label="Menu">
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <nav className="px-6 py-6 flex flex-col gap-1">
              ...
              <div className="flex items-center gap-4 mt-4">
                <button onClick={toggle}
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-petrol/15 text-[13px] font-mono font-medium text-petrol">
                  <span className={isDE ? "text-petrol font-bold" : "text-petrol/40"}>DE</span>
                  <span className="text-petrol/20 mx-0.5">|</span>
                  <span className={!isDE ? "text-petrol font-bold" : "text-petrol/40"}>EN</span>
                </button>
                <Link href="/kontakt" onClick={() => setOpen(false)}
                  className="inline-flex items-center justify-center gap-2 bg-petrol text-pearl px-5 py-3 rounded-full text-sm font-medium flex-1">
                  {isDE ? "Anfrage stellen" : "Get in touch"}
                </Link>
              </div>
            </nav>

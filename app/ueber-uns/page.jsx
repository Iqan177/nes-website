  ...
  return (
    <>
      <section className="pt-32 lg:pt-40 pb-16 lg:pb-20">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <h1 className="font-display font-medium text-5xl md:text-6xl lg:text-7xl xl:text-8xl tracking-tightest leading-[0.98] text-petrol max-w-5xl">
            {isDE ? <>Wir liefern<br /><span className="text-petrol/60">Energiezukunft.</span></> 
          ...
          <p className="mt-8 max-w-2xl text-lg lg:text-xl text-petrol/70 leading-relaxed">
            {isDE
              ? "NES Energy Group vertreibt maßgeschneiderte Sodium-Ionen Batteriespeicher-Container — schlüsselfertig, mit After Sales & Kundenbetreuung direkt aus Deutschland."
              : "NES Energy Group supplies customised sodium-ion battery storage containers — turnkey, with after sales & customer support directly from Germany."}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-7 lg:pt-3 space-y-5 text-lg text-petrol/80 leading-relaxed">
              <p>
                {isDE
                  ? "Energiespeicher sind der Schlüssel zur Energiewende. NES bietet eine sichere Alternative: Sodium-Ionen-Container, die umweltfreundlich und bis zu 20 % günstiger als Lithium-Ionen-Systeme sind."
                  : "Energy storage is the key to the energy transition. NES offers a safe alternative: sodium-ion containers that are environmentally friendly and up to 20% cheaper than lithium-ion systems."}
              </p>
              <p className="text-petrol font-medium">
                {isDE ? "Maßgeschneidert. Schlüsselfertig. Zuverlässig." : "Customised. Turnkey. Reliable."}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            <div className="lg:col-span-5">
              <SectionHeader
                ...
                description={isDE ? "Im Herzen Nordwestdeutschlands — bestens vernetzt für den deutschlandweiten Service." : "In the heart of north-west Germany — ideally connected for nationwide service."}
              />
      ...
      <section className="py-24 lg:py-32">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <div className="mt-14 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {TEAM.map((member, i) => (
              <motion.div key={member.name} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="space-y-2 pt-4 border-t border-petrol/10">
                  {member.email && (
                    <a href={`mailto:${member.email}`} className="flex items-center gap-2.5 text-[12px] text-petrol/60 hover:text-petrol transition-colors break-all">
                      <Icon name="mail" className="w-3.5 h-3.5 text-cyan-700 shrink-0" />{member.email}
                    </a>
                  )}
                  {member.phone && (
                    <a href={`tel:${member.phone.replace(/\s/g, "")}`} className="flex items-center gap-2.5 text-[12px] text-petrol/60 hover:text-petrol transition-colors">
                      <Icon name="phone" className="w-3.5 h-3.5 text-cyan-700 shrink-0" />{member.phone}
                    </a>
                  )}
                </div>
              </motion.div>
      ...
      <CTASection
        ...
        description={isDE ? "Wir freuen uns auf jede Anfrage — egal ob Großprojekt oder erste Fragen." : "We look forward to every enquiry — whether large project or initial questions."}
      />
    </>
  );
}
...
function MapVisual() {
  return (
    <svg viewBox="0 0 600 450" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
      ...
      <path d="M 250 60 L 300 50 L 360 70 L 390 90 L 400 130 L 420 140 L 430 170 L 410 200 L 430 230 L 420 260 L 400 280 L 380 310 L 350 340 L 310 350 L 280 360 L 250 350 L 220 330 L 200 300 L 180 280 L 170 250 L 180 220 L 160 190 L 170 160 L 190 140 L 200 110 L 220 80 Z"
      ...
      <g transform="translate(215, 155)">
        ...
        <circle r="18"><animate attributeName="r" values="18;30;18" dur="2.5s" repeatCount="indefinite" />
      ...
      <text x="240" y="174" fill="rgba(247,244,238,0.5)" fontFamily="Geist Mono, monospace" fontSize="9">Niedersachsen</text>

...
export async function POST(request) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ success: false, error: "Konfigurationsfehler." }, { status: 500 });
    }

    // Resend wird HIER initialisiert — nicht außerhalb
...
<body style="margin:0;padding:24px;background:#F7F4EE;font-family:Arial,sans-serif">
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(10,37,64,0.08)">
  <div style="background:#0A2540;padding:36px 40px">
    ...
    <h1 style="margin:0;font-size:24px;font-weight:700;color:#F7F4EE">Neue ${config ? "Konfigurator-Anfrage" : "Kontaktanfrage"}</h1>
  </div>
  <div style="padding:36px 40px">
    <table style="width:100%;border-collapse:collapse">${rows}</table>
    ${message ? `<div style="margin-top:24px;padding:20px;background:#F7F4EE;border-radius:10px"><p style="margin:0 0 8px;font-size:11px;text-transform:uppercase;letter-spacing:0.15em;color:#6B675F;font-family:monospace">Nachricht</p><p style="margin:0;font-size:14px;color:#0A2540;line-height:1.7">${message.replace(/\n/g, "<br>")}</p></div>` : ""}
    <div style="margin-top:28px"><a href="mailto:${email}" style="display:inline-block;background:#0A2540;color:#F7F4EE;padding:12px 28px;border-radius:50px;text-decoration:none;font-size:14px;font-weight:600">Direkt antworten →</a></div>
  </div>
  <div style="padding:20px 40px;background:#F7F4EE;border-top:1px solid #EFE9DD"><p style="margin:0;font-size:11px;color:#9E9A91;font-family:monospace">Next Energy Solution · info@nes-energygroup.com</p></div>
</div></body></html>`;
}
...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(10,37,64,0.08)">
  <div style="background:#0A2540;padding:36px 40px">
    <p style="margin:0 0 6px;font-size:11px;letter-spacing:0.25em;text-transform:uppercase;color:#00D4D8;font-family:monospace">Next Energy Solution</p>
    <h1 style="margin:0;font-size:24px;font-weight:700;color:#F7F4EE">Ihre Anfrage ist eingegangen.</h1>
  </div>
  <div style="padding:40px 40px">
    <p style="font-size:18px;font-weight:600;color:#0A2540;margin:0 0 16px">Sehr geehrte/r ${firstName},</p>
    <p style="font-size:15px;color:#4A4842;line-height:1.7;margin:0 0 16px">vielen Dank für Ihre ${isConfig ? "Konfigurator-Anfrage" : "Anfrage"} bei <strong>Next Energy Solution</strong>.</p>
    <p style="font-size:15px;color:#4A4842;line-height:1.7;margin:0 0 24px">Wir haben Ihre Nachricht erhalten und werden uns schnellstmöglich bei Ihnen melden.</p>
    ...
    <p style="font-size:15px;color:#4A4842;line-height:1.7;margin:0">Mit freundlichen Grüßen,<br><strong>Next Energy Solution</strong></p>
  </div>
  <div style="padding:20px 40px;background:#F7F4EE;border-top:1px solid #EFE9DD"><p style="margin:0;font-size:11px;color:#9E9A91;font-family:monospace">Next Energy Solution · info@nes-energygroup.com</p></div>
</div></body></html>`;
}
